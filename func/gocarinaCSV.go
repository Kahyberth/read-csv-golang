...
func ReadCSVOcarina() ([]model.User, time.Duration, error) {
	start := time.Now()

	originalFile, err := os.ReadFile("clientes.csv")
	if err != nil {
		return nil, 0, fmt.Errorf("error al leer archivo original: %v", err)
	}
	...
	if err != nil {
		return nil, 0, fmt.Errorf("error al escribir archivo temporal: %v", err)
	}

	file, err := os.Open(tempFilePath)
	if err != nil {
		return nil, 0, fmt.Errorf("error al abrir archivo temporal: %v", err)
	}
	...
	if err := gocsv.UnmarshalFile(file, &users); err != nil {
		return nil, 0, fmt.Errorf("error al parsear CSV: %v", err)
	}

	gocarinaTime := time.Since(start)
